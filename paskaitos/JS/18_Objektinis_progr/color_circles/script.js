...
class Circle {
  ...
  static init(circlesPlace, buttonsPlace, count = 5) {
    ...
    this.createButton(
      buttonsPlace,
      'Move left',
      this.effectMoveLeft.bind(this)
    );
    this.createButton(
      buttonsPlace,
      'Move right',
      this.effectMoveRight.bind(this)
    );
    this.createButton(buttonsPlace, 'Stop', _ =>
  ...
  static effectMoveLeft() {
    ...
    this.effectInterval = setInterval(_ => {
      ...
      this.circles.forEach(circle =>
        this.circlesPlace.appendChild(circle.circleDiv)
      );
    }, 700);
  }
...
class Colors4 extends Circle {
  ...
  randomColor() {
    const randomIndex = Math.floor(
      Math.random() * this.constructor.colors.length
    );
